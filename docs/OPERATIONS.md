...
| `BAZUSOP_BOOTSTRAP_SECRET` | Mutasyonlar için evet | İlk platform yöneticisini oluşturmak için tek kullanımlık secret |
| `BAZUSOP_TOTP_ENCRYPTION_KEY` | Mutasyonlar için evet | TOTP secret'larını veritabanında şifrelemek için runtime anahtarı |
| `BAZUSOP_SERVICE_ACCOUNT_PEPPER` | Servis hesapları için evet | Servis hesabı token'larını HMAC ile özetlemek için veritabanı dışı pepper |
| `BAZUSOP_TRUSTED_ORIGINS` | Hayır | CORS için güvenilir origin listesi (virgülle ayrılmış) |
...
`BAZUSOP_BOOTSTRAP_SECRET`/`BAZUSOP_TOTP_ENCRYPTION_KEY` ayarlanmadığında local
identity (bootstrap/davetler) devre dışı kalır ve — spike 11.7'nin kaldırdığı
eski operator/admin bearer köprüsü artık mevcut olmadığından — mutasyon içeren
hiçbir API isteği kimlik doğrulanamaz. Bkz.
[docs/MIGRATION_v0.4.md](MIGRATION_v0.4.md).
...
make test
make build
BAZUSOP_ENROLLMENT_TOKEN=local-token BAZUSOP_BOOTSTRAP_SECRET=local-bootstrap-secret BAZUSOP_TOTP_ENCRYPTION_KEY=local-totp-key ./bin/bazusop-hub
```

Tam stack için:

```bash
POSTGRES_PASSWORD=yerel-parola BAZUSOP_ENROLLMENT_TOKEN=yerel-token BAZUSOP_BOOTSTRAP_SECRET=yerel-bootstrap-secret BAZUSOP_TOTP_ENCRYPTION_KEY=yerel-totp-anahtari docker compose up --build
```
...
Chart uygulama secret’ını üretmez. `database-url`, `enrollment-token` ve isteğe
bağlı `bootstrap-secret`, `totp-encryption-key`, `service-account-pepper`
anahtarlarını taşıyan mevcut bir Secret verilmelidir. Bu üç isteğe bağlı anahtar
olmadan local identity/servis hesapları devre dışı kalır ve mutasyon içeren
hiçbir API isteği kimlik doğrulanamaz (bkz.
[docs/MIGRATION_v0.4.md](MIGRATION_v0.4.md)). TLS etkinse server cert/key ayrıca
mevcut bir TLS Secret’tan read-only mount edilir.
...
4. Enrollment token'ı, bootstrap secret'ı, TOTP şifreleme anahtarını ve servis
   hesabı pepper'ını ayrı, yüksek entropili değerlerle oluştur; secret erişimini
   sınırla ve rotasyon prosedürünü test et.
...
- Herhangi bir mutasyon `401` dönüyorsa geçerli bir oturum çerezi veya servis
  hesabı token'ı (`Authorization: Bearer bazusop_sat_...`) kullanıldığını
  kontrol et; bkz. [docs/MIGRATION_v0.4.md](MIGRATION_v0.4.md). UI token'ı
  kalıcı depolamaz ve başarılı oluşturmadan sonra bellekten temizler.
- Mutasyon `403` dönüyorsa çağıranın ilgili izne (ör. `PermissionManageAlerts`,
  `PermissionCreateJobs`) sahip bir site rolüyle kimlik doğruladığını kontrol et
  — bir viewer veya operator rolü, site-admin gerektiren eylemler için reddedilir.
